module switchboard_std::aggregator {
    ...
    #[test_only]
    public fun destroy_aggregator_for_testing(
        aggregator: Aggregator
    ) {
        let Aggregator {
            id,
            ...
            min_oracle_results:  _min_oracle_results,
            min_update_delay_seconds:  _min_update_delay_seconds,
            name:  _name,
            history_limit:  _history_limit,
            variance_threshold:  _variance_threshold,
            force_report_period:  _force_report_period,
            min_job_results:  _min_job_results,
            crank_disabled:  _crank_disabled,
            crank_row_count:  _crank_row_count,
            next_allowed_update_time:  _next_allowed_update_time,
            created_at:  _created_at,
            read_charge:  _read_charge,
            reward_escrow:  _reward_escrow,
            read_whitelist,
            limit_reads_to_whitelist:  _limit_reads_to_whitelist,
            update_data,
            interval_id:  _interval_id,
            curr_interval_payouts:  _curr_interval_payouts,
            next_interval_refresh_time:  _next_interval_refresh_time,
            escrows,
            friend_key:  _friend_key,
            version:  _version,
        } = aggregator;

        bag::destroy_empty(read_whitelist);
        bag::destroy_empty(escrows);
